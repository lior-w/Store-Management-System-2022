...
public class EmployeeDAO extends DAO<EmployeeDTO> {
    ...
    public EmployeeDTO makeDTO(String ID)  {
        ...
        try {
            ...
            output = new EmployeeDTO(/*first name*/RS.getString(1), /*last name*/RS.getString(2), /*Id*/RS.getString(3),
                    ...
                    /*start working date*/LocalDate.parse(RS.getString(7)), skills, availableShifts);
        } catch (Exception e) {
            output = null;
        }
    ...
    public int update(EmployeeDTO updatedOb) throws SQLException//not allowed to change ID
    {
        Connection conn = Repository.getInstance().connect();
        ...
        String updateString = String.format("UPDATE %s" +
                ...
                updatedOb.bankAccount, updatedOb.salary, updatedOb.empConditions, updatedOb.startWorkingDate, updatedOb.id);
        Statement s;
        try {
            s = conn.createStatement();
